...
Usage:
    python3 harness/run_trial_temporal_frequency_required.py --model claude-sonnet-4-6 \
...
from lookup_tool_schemas import LOOKUP_FREQUENCY_TOOL
from lookup_tools import execute_lookup_frequency
...
CONDITION = "temporal_frequency_required"
...
REQUIRED_LOOKUP_TOOLS = {"lookup_frequency"}
MAX_SUBMIT_REJECTIONS = 2
SUBMIT_REJECTION_MESSAGE = (
    "Submission rejected: you must call lookup_frequency "
...
NO_TOOL_CALL_REJECTION_MESSAGE = (
    "You stopped without calling lookup_frequency or submit_answer. "
    "You must call lookup_frequency before this trial can end — call it on "
...
GATE_REMINDER_CLAUSE = (
    ...
    "lookup_frequency at least once — if you haven't yet, call it now so "
...
Use list_files, read_file, search, and lookup_frequency to investigate the issue. Once you are reasonably confident you have identified the correct file(s), call submit_answer — you do not need to exhaustively verify every hypothesis. Focus on source files, not tests or documentation. Before submit_answer will be accepted, you must call lookup_frequency — an attempt to submit without doing so will be rejected so you can call it and try again.\
"""
