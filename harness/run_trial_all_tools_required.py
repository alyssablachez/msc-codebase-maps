...
Usage:
    python3 harness/run_trial_all_tools_required.py --model claude-sonnet-4-6 \
...
from lookup_tool_schemas import LOOKUP_STRUCTURE_TOOL, LOOKUP_FREQUENCY_TOOL, LOOKUP_COCHANGE_TOOL
from lookup_tools import execute_lookup_structure, execute_lookup_frequency, execute_lookup_cochange
...
REQUIRED_LOOKUP_TOOLS = {"lookup_structure", "lookup_frequency", "lookup_cochange"}
MAX_SUBMIT_REJECTIONS = 2
SUBMIT_REJECTION_MESSAGE = (
    "Submission rejected: you must call at least one of lookup_structure, lookup_frequency, or lookup_cochange "
...
NO_TOOL_CALL_REJECTION_MESSAGE = (
    "You stopped without calling any lookup tool or submit_answer. "
    "You must call at least one of lookup_structure, lookup_frequency, or "
    "lookup_cochange before this trial can end — call it on a file you "
    "suspect is relevant, then call submit_answer once you're confident."
)
...
GATE_REMINDER_CLAUSE = (
    " Also: submit_answer will be rejected until you've called at least "
    "one of lookup_structure, lookup_frequency, or lookup_cochange — if "
    "you haven't yet, call one now so you don't waste a turn on a "
    "rejected submission."
)
...
def execute_tool(name, args, repo_dir, repo_name, issue_idx, maps_root):
    try:
        ...
        elif name == "lookup_structure":
            return execute_lookup_structure(args, repo_name, issue_idx, maps_root)
        elif name == "lookup_frequency":
            return execute_lookup_frequency(args, repo_name, issue_idx, maps_root)
...
- lookup_structure(path, offset=None): Look up a file's structural signature -- every class and function, with line numbers, full signatures, and full docstrings -- without reading the whole file. Large files are paginated, not truncated; if the response says more is available, call again with the suggested offset to see the rest. Use it to narrow down whether a file looks relevant based on its class/function names and signatures, before committing to a full read_file call.
- lookup_frequency(path): Look up how many historical commits have touched a file (up to the commit this issue was filed against) and the date of its most recent edit. Frequent edits are a weak signal of active/important code, not proof of relevance to this specific issue -- a file may be generic and change often for unrelated reasons, and the correct file may be edited rarely. Use it alongside your other investigation, not as a substitute for it.
...
Use list_files, read_file, search, lookup_structure, lookup_frequency, and lookup_cochange to investigate the issue. Once you are reasonably confident you have identified the correct file(s), call submit_answer — you do not need to exhaustively verify every hypothesis. Focus on source files, not tests or documentation. Before submit_answer will be accepted, you must call at least one of lookup_structure, lookup_frequency, or lookup_cochange — an attempt to submit without doing so will be rejected so you can call it and try again.\
"""
