...
Usage:
    python3 harness/run_trial_structural_required.py --model claude-sonnet-4-6 \
...
from lookup_tool_schemas import LOOKUP_STRUCTURE_TOOL
from lookup_tools import execute_lookup_structure
...
REQUIRED_LOOKUP_TOOLS = {"lookup_structure"}
MAX_SUBMIT_REJECTIONS = 2
SUBMIT_REJECTION_MESSAGE = (
    "Submission rejected: you must call lookup_structure "
...
Use list_files, read_file, search, and lookup_structure to investigate the issue. Once you are reasonably confident you have identified the correct file(s), call submit_answer — you do not need to exhaustively verify every hypothesis. Focus on source files, not tests or documentation. Before submit_answer will be accepted, you must call lookup_structure — an attempt to submit without doing so will be rejected so you can call it and try again.\
"""
