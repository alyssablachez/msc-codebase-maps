"""
Run a full batch of submit-gated ("required") tool-based-retrieval trials
for one (worker, rep, model) combination: every issue in
issue_selection_final.csv, across the four required-condition harness
copies (structural_required, temporal_frequency_required,
temporal_cochange_required, all_tools_required). See DEVLOG 2026-07-23
for the submit-gate design (reject submit_answer until a required
lookup_* tool has been called, capped at MAX_SUBMIT_REJECTIONS).

Sibling of run_batch_tools.py, not a --flag on it -- same rationale as
the harness files themselves being separate per-condition copies rather
than one shared file: dispatches to a different required-condition
harness script per condition via HARNESS_FOR_CONDITION.

Condition names are distinct strings from both Study 1's map_type values
and Study 2's voluntary condition names, so all three coexist in the same
results/ tree without collision.

Trials whose result file already exists are skipped (safe to re-run after
a partial batch). Trials whose required full-index file(s) are missing
are skipped with a warning rather than failing.

Usage:
    python3 scripts/run_batch_tools_required.py --worker-id 3 --rep 1 \
        --model mistral/ministral-3b-latest

    python3 scripts/run_batch_tools_required.py --worker-id 3 --rep 1 \
        --model mistral/ministral-3b-latest --dry-run
"""
import argparse
import datetime
import json
...
# Condition -> harness script. Each is a full, single-condition copy
# generated from the voluntary run_trial_{condition}.py (see DEVLOG
# 2026-07-23), with a submission gate added -- not a shared file with a
# flag, matching the voluntary harnesses' own precedent.
HARNESS_FOR_CONDITION = {
    "structural_required":         os.path.join(HARNESS_DIR, "run_trial_structural_required.py"),
    "temporal_frequency_required": os.path.join(HARNESS_DIR, "run_trial_temporal_frequency_required.py"),
    "temporal_cochange_required":  os.path.join(HARNESS_DIR, "run_trial_temporal_cochange_required.py"),
...
# {maps_base}/{repo}/{issue_idx}/ for a trial to be runnable -- same
# underlying index files as the voluntary conditions, just keyed by the
# _required condition name.
REQUIRED_INDEX_FILES = {
    "structural_required":         ["ast_index_full.json"],
    "temporal_frequency_required": ["freq_index_full.json"],
    "temporal_cochange_required":  ["cochange_index_full.json"],
    "all_tools_required":          ["ast_index_full.json", "freq_index_full.json", "cochange_index_full.json"],
}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run a full batch of required-condition trials for one worker/rep/model")
    ...
    if args.dry_run:
        print(f"DRY RUN (required)  worker={args.worker_id} rep={args.rep} model={args.model}")
    ...
    batch_log_path = os.path.join(
        LOGS_DIR, f"batch_tools_required_{args.worker_id}_{args.rep}_{safe_model}_{date_str}.log")
    ...
    log("=" * 72)
    log(f"BATCH (required tool conditions)  worker={args.worker_id} rep={args.rep} model={args.model}")
