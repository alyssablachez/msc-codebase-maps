"""
Study 1 -- does map condition move frac_gt_touched (data/gt_files_touched_
...
scripts/stats_study1_secondary_nonparametric.py.

Usage:
    python3 scripts/stats_study1_frac_gt_touched.py
"""
import os
...
def main():
    ...
    df = load_data()
    print(f"Study 1, multi-file-issue subset: {len(df)} trials, "
    ...
    friedman_result.to_csv(os.path.join(OUT_DIR, "study1_frac_gt_touched_friedman.csv"), index=False)
    wilcoxon_pooled.to_csv(os.path.join(OUT_DIR, "study1_frac_gt_touched_wilcoxon_pooled.csv"), index=False)
    wilcoxon_per_model.to_csv(os.path.join(OUT_DIR, "study1_frac_gt_touched_wilcoxon_per_model.csv"), index=False)
    print(f"\nSaved CSVs -> {OUT_DIR}/")
