"""
Study 3 -- same two new process metrics as scripts/stats_study1_touch_timing.py
("explored" via Cochran's Q + McNemar, first-touch timing via a stratified
Cox model), applied to Study 3's data. See that script's docstring for the
...
the shared cross-study control, same as scripts/stats_study3_f1.py and
scripts/stats_study3_secondary_nonparametric.py.

Usage:
    python3 scripts/stats_study3_touch_timing.py
"""
import os
...
def main():
    ...
    df = load_data()
    print(f"Study 3 (+ shared baseline) sample: {len(df)} trials, explored rate = {df['explored'].mean():.3f}")
    ...
    q_result.to_csv(os.path.join(OUT_DIR, "study3_explored_cochrans_q.csv"), index=False)
    mcnemar_pooled.to_csv(os.path.join(OUT_DIR, "study3_explored_mcnemar_pooled.csv"), index=False)
    mcnemar_per_model.to_csv(os.path.join(OUT_DIR, "study3_explored_mcnemar_per_model.csv"), index=False)

    cox_omnibus = run_cox_omnibus(df)
    cox_pooled = run_cox_pairwise_pooled(df)
    cox_per_model = run_cox_pairwise_per_model(df)
    cox_omnibus.to_csv(os.path.join(OUT_DIR, "study3_first_touch_cox_omnibus.csv"), index=False)
    cox_pooled.to_csv(os.path.join(OUT_DIR, "study3_first_touch_cox_pairwise_pooled.csv"), index=False)
    cox_per_model.to_csv(os.path.join(OUT_DIR, "study3_first_touch_cox_pairwise_per_model.csv"), index=False)
