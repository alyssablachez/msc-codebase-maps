"""
Study 2 -- nonparametric robustness checks for the secondary/process/cost
metrics, alongside the R GLMMs in scripts/stats_study2_r_secondary.R
(turns, turn-cap) and scripts/stats_study2_r_tokens_cost_walltime.R
...
(scripts/stats_study2_success.py) -- same structure as Study 1's version
of this script.

Study 2 has no baseline condition of its own -- study==1's "baseline" is
the shared cross-study control; see scripts/stats_study2_f1.py's docstring.

Primary sample only: rep<=3.

Usage:
    python3 scripts/stats_study2_secondary_nonparametric.py
"""
import os
...
def main():
    ...
    friedman_result.to_csv(os.path.join(OUT_DIR, "study2_secondary_friedman.csv"), index=False)
    wilcoxon_pooled.to_csv(os.path.join(OUT_DIR, "study2_secondary_wilcoxon_pooled.csv"), index=False)
    wilcoxon_per_model.to_csv(os.path.join(OUT_DIR, "study2_secondary_wilcoxon_per_model.csv"), index=False)
    ...
    q_result.to_csv(os.path.join(OUT_DIR, "study2_turncap_cochrans_q.csv"), index=False)
    mcnemar_pooled.to_csv(os.path.join(OUT_DIR, "study2_turncap_mcnemar_pooled.csv"), index=False)
    mcnemar_per_model.to_csv(os.path.join(OUT_DIR, "study2_turncap_mcnemar_per_model.csv"), index=False)
    print(f"\nSaved CSVs -> {OUT_DIR}/")
