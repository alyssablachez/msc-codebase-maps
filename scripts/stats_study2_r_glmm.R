#!/usr/bin/env Rscript
# Study 2 (on-demand/voluntary map tools vs. baseline) -- the "real" GLMM
# fits, same structure and families as Study 1's scripts/stats_study1_r_glmm.R:
#   - F1: glmmTMB, ordbeta() family.
#   - success: lme4::glmer, binomial family.
# Both with crossed random effects (1|issue_id) + (1|repo). Study 2 has 5
# map_condition levels (baseline + structural/temporal_frequency/
# temporal_cochange/all_tools) vs. Study 1's 4, since study==1's baseline
# is the shared cross-study control (see scripts/stats_study2_f1.py).
#
# Usage:
#   Rscript scripts/stats_study2_r_glmm.R
...
write.csv(lrt_out, file.path(out_dir, "study2_r_lrt_summary.csv"), row.names = FALSE)
...
cat(" -", file.path(out_dir, "study2_r_lrt_summary.csv"), "\n")

saveRDS(list(f1_full = f1_full, f1_reduced = f1_reduced,
            succ_full = succ_full, succ_reduced = succ_reduced),
       file.path(out_dir, "study2_r_models.rds"))
cat(" -", file.path(out_dir, "study2_r_models.rds"), "(fitted model objects, for diagnostics)\n")
