"""
Study 2 (on-demand/voluntary map tools) -- how many trials actually
called a map-lookup tool at least once, broken down by treatment
(map_condition) and, within each, by model. Tool use is voluntary in
Study 2, so this is descriptive: not every trial that had a tool
available chose to call it.

Primary sample only: study==2, rep<=3 (2160 trials, 540 per condition).
Reads each trial's raw JSON via its source_path (see scripts/tool_usage_common.py)
rather than anything precomputed, since tool-call detail isn't part of
data/compiled_results_combined.pkl's columns.

Usage:
    python3 scripts/study2_tool_usage_by_treatment.py
"""
import os
...
OUT_CSV = os.path.join(OUT_DIR, "study2_tool_usage_by_treatment.csv")
