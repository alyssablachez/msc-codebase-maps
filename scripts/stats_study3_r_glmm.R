#!/usr/bin/env Rscript
# Study 3 (on-demand/required map tools vs. baseline) -- the "real" GLMM
# fits, same structure and families as Study 1/2's scripts/stats_study{1,2}_r_glmm.R:
#   - F1: glmmTMB, ordbeta() family.
#   - success: lme4::glmer, binomial family.
# Both with crossed random effects (1|issue_id) + (1|repo). Study 3 has 5
# map_condition levels (baseline + structural/temporal_frequency/
# temporal_cochange/all_tools), mirroring Study 2's conditions but with
# submission_mode="required" instead of "voluntary" -- study==1's baseline
# is the shared cross-study control (see scripts/stats_study3_f1.py).
#
# Usage:
#   Rscript scripts/stats_study3_r_glmm.R
...
data_csv <- file.path(root, "data", "stats_study3", "study3_for_r.csv")
out_dir <- file.path(root, "data", "stats_study3")
...
cat(sprintf("Study 3 (+shared baseline) sample: %d trials, %d issues, %d repos, %d models\n",
...
save_glmmTMB_coefs <- function(model, outcome, suffix) {
  ...
  write.csv(coef_df[, c("term", "estimate", "std_error", "z_value", "p_value")],
            file.path(out_dir, paste0("study3_r_", outcome, "_", suffix, "_coefficients.csv")), row.names = FALSE)
}
...
tryCatch(print(as.data.frame(VarCorr(f1_reduced))), error = function(e) {
...
write.csv(lrt_out, file.path(out_dir, "study3_r_lrt_summary.csv"), row.names = FALSE)

cat("\n", strrep("=", 70), "\nSaved:\n", strrep("=", 70), "\n", sep = "")
for (outcome in c("f1", "success")) {
  for (suffix in c("reduced", "full")) {
    cat(" -", file.path(out_dir, paste0("study3_r_", outcome, "_", suffix, "_coefficients.csv")), "\n")
  }
}
cat(" -", file.path(out_dir, "study3_r_lrt_summary.csv"), "\n")

saveRDS(list(f1_full = f1_full, f1_reduced = f1_reduced,
            succ_full = succ_full, succ_reduced = succ_reduced),
       file.path(out_dir, "study3_r_models.rds"))
cat(" -", file.path(out_dir, "study3_r_models.rds"), "(fitted model objects, for diagnostics)\n")
