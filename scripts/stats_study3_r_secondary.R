#!/usr/bin/env Rscript
# Study 3 -- turns_used (glmmTMB, nbinom2) and hit_turn_cap (lme4::glmer,
# binomial), same structure as scripts/stats_study1_r_secondary.R and
# scripts/stats_study2_r_secondary.R.
#
# Usage:
#   Rscript scripts/stats_study3_r_secondary.R
...
write.csv(lrt_out, file.path(out_dir, "study3_r_secondary_lrt_summary.csv"), row.names = FALSE)
...
cat(" -", file.path(out_dir, "study3_r_secondary_lrt_summary.csv"), "\n")

saveRDS(list(turns_full = turns_full, turns_reduced = turns_reduced,
            cap_full = cap_full, cap_reduced = cap_reduced),
       file.path(out_dir, "study3_r_secondary_models.rds"))
cat(" -", file.path(out_dir, "study3_r_secondary_models.rds"), "(fitted model objects, for diagnostics)\n")
