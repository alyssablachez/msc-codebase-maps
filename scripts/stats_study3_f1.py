"""
Study 3 (on-demand/required map tools vs. baseline), F1 side of the
combined-chapter stats battery -- the nonparametric tests (2-4 in the
original numbering), same structure as Study 1's
scripts/stats_study1_f1.py and Study 2's scripts/stats_study2_f1.py, run
...
scripts/stats_study3_r_glmm.R -- see stats_study1_f1.py's docstring for
why the Python GEE/marginal-LRT version of that test was dropped rather
than kept alongside the R fit.

Study 3 mirrors Study 2's conditions exactly (structural/temporal_frequency/
temporal_cochange/all_tools, all delivery_mechanism="on_demand") but with
submission_mode="required" instead of "voluntary" -- tool use is mandated
rather than optional. Study 3 has no baseline condition of its own --
study==1's "baseline" is the shared cross-study control (user-confirmed
design decision; see scripts/collate_studies_1_2_3.py), so the sample here
is Study 3's 4 conditions (2160 trials) plus Study 1's baseline (540
trials) = 2700.
...
Usage:
    python3 scripts/stats_study3_f1.py
"""
import os
...
def main():
    ...
    friedman_result.to_csv(os.path.join(OUT_DIR, "study3_f1_friedman.csv"), index=False)
    wilcoxon_pooled.to_csv(os.path.join(OUT_DIR, "study3_f1_wilcoxon_pooled.csv"), index=False)
    wilcoxon_per_model.to_csv(os.path.join(OUT_DIR, "study3_f1_wilcoxon_per_model.csv"), index=False)
    print(f"\nSaved CSVs -> {OUT_DIR}/")
