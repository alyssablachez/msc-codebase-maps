#!/usr/bin/env Rscript
# Study 3 -- total_input_tokens, total_cost, wall_time_seconds, all via
# glmmTMB Gamma(link="log"), crossed random effects, same structure and
# same rationale as scripts/stats_study1_r_tokens_cost_walltime.R and
# scripts/stats_study2_r_tokens_cost_walltime.R (see Study 1's script
# docstring for why Gamma-log is the right family here). The
# Friedman/Wilcoxon version (scripts/stats_study3_secondary_nonparametric.py)
# is the secondary robustness check, not the primary test.
#
# Usage:
#   Rscript scripts/stats_study3_r_tokens_cost_walltime.R
...
fit_one <- function(outcome_col, out_dir) {
  ...
  save_coefs <- function(model, suffix) {
    ...
    write.csv(coef_df[, c("term", "estimate", "std_error", "z_value", "p_value")],
              file.path(out_dir, paste0("study3_r_", outcome_col, "_", suffix, "_coefficients.csv")), row.names = FALSE)
  }
...
write.csv(lrt_out, file.path(out_dir, "study3_r_tokens_cost_walltime_lrt_summary.csv"), row.names = FALSE)

cat("\n", strrep("=", 70), "\nSaved:\n", strrep("=", 70), "\n", sep = "")
for (outcome_col in c("total_input_tokens", "total_cost", "wall_time_seconds")) {
  for (suffix in c("reduced", "full")) {
    cat(" -", file.path(out_dir, paste0("study3_r_", outcome_col, "_", suffix, "_coefficients.csv")), "\n")
  }
}
cat(" -", file.path(out_dir, "study3_r_tokens_cost_walltime_lrt_summary.csv"), "\n")
...
saveRDS(list(tokens_full = tokens_result$full, tokens_reduced = tokens_result$reduced,
       ...
       file.path(out_dir, "study3_r_tokens_cost_walltime_models.rds"))
cat(" -", file.path(out_dir, "study3_r_tokens_cost_walltime_models.rds"), "(fitted model objects, for diagnostics)\n")
