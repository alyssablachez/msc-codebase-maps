#!/usr/bin/env Rscript
# Study 1 -- test 10 (revised), same rigor level as the rest of the
# battery (stats_study1_r_glmm.R for F1/success, stats_study1_r_secondary.R
# for turns/turn-cap): total_input_tokens, total_cost, and
# wall_time_seconds all get glmmTMB, Gamma(link="log") family, crossed
# random effects (1|issue_id) + (1|repo), full (3-way interaction) vs.
# reduced (all 2-way interactions) LRT.
#
# Gamma-with-log-link is the standard choice for continuous, strictly
# positive, right-skewed data like this (all three metrics have min > 0
# -- verified in Python before writing this script), the same logic that
# picked ordbeta for F1 and nbinom2 for turns: match the family to the
# data's actual support and shape rather than defaulting to a nonparametric
# test. The original Friedman/Wilcoxon battery
# (scripts/stats_study1_secondary_nonparametric.py) is kept as a
# secondary robustness check, not the primary test -- it doesn't
# simultaneously model the issue_id/repo crossed structure or let the
# 3-way interaction be tested the way the GLMM does.
#
# Usage:
#   Rscript scripts/stats_study1_r_tokens_cost_walltime.R
...
fit_one <- function(outcome_col, outcome_label, out_dir) {
  cat("\n", strrep("=", 70), "\n", outcome_label, " -- glmmTMB, Gamma(link=\"log\"), crossed random effects\n", strrep("=", 70), "\n", sep = "")
  ...
  save_coefs <- function(model, suffix) {
    ...
    write.csv(coef_df[, c("term", "estimate", "std_error", "z_value", "p_value")],
              file.path(out_dir, paste0("study1_r_", outcome_col, "_", suffix, "_coefficients.csv")), row.names = FALSE)
  }
...
tokens_result <- fit_one("total_input_tokens", "total_input_tokens", out_dir)
cost_result <- fit_one("total_cost", "total_cost", out_dir)
walltime_result <- fit_one("wall_time_seconds", "wall_time_seconds", out_dir)

lrt_out <- rbind(tokens_result$lrt_row, cost_result$lrt_row, walltime_result$lrt_row)
write.csv(lrt_out, file.path(out_dir, "study1_r_tokens_cost_walltime_lrt_summary.csv"), row.names = FALSE)

cat("\n", strrep("=", 70), "\nSaved:\n", strrep("=", 70), "\n", sep = "")
for (outcome_col in c("total_input_tokens", "total_cost", "wall_time_seconds")) {
  for (suffix in c("reduced", "full")) {
    cat(" -", file.path(out_dir, paste0("study1_r_", outcome_col, "_", suffix, "_coefficients.csv")), "\n")
  }
}
cat(" -", file.path(out_dir, "study1_r_tokens_cost_walltime_lrt_summary.csv"), "\n")
...
saveRDS(list(tokens_full = tokens_result$full, tokens_reduced = tokens_result$reduced,
       ...
       file.path(out_dir, "study1_r_tokens_cost_walltime_models.rds"))
cat(" -", file.path(out_dir, "study1_r_tokens_cost_walltime_models.rds"), "(fitted model objects, for diagnostics)\n")
