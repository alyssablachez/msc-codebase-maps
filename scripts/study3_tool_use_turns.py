"""
Study 3 -- average number of map-lookup-tool calls per trial, conditional
on the trial having used the tool at least once. Same structure as
scripts/study2_tool_use_turns.py; the interesting contrast is whether
required trials, once they comply with the gate, stop at exactly one
call (minimal compliance) or continue using the tool as Study 2's
voluntary trials did.

Primary sample only: study==3, rep<=3.

Usage:
    python3 scripts/study3_tool_use_turns.py
"""
import os
...
OUT_CSV = os.path.join(OUT_DIR, "study3_tool_use_turns.csv")
