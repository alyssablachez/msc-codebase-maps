"""
Study 2 (on-demand/voluntary map tools vs. baseline), success (binary =
...
Study 1's scripts/stats_study1_success.py, run against Study 2's own
conditions:
...
scripts/stats_study2_r_glmm.R -- see stats_study1_success.py's docstring
...
convention as scripts/stats_study2_f1.py -- run that one first for the F1
results this is meant to sit alongside as a robustness check.

Usage:
    python3 scripts/stats_study2_success.py
"""
import os
...
def main():
    ...
    df = load_study2_data()
    print(f"Study 2 (+ shared baseline) sample: {len(df)} trials, success rate = {df['success_int'].mean():.3f}")

    q_result = run_cochrans_q(df)
    mcnemar_pooled = run_mcnemar_pooled(df)
    mcnemar_per_model = run_mcnemar_per_model(df)

    q_result.to_csv(os.path.join(OUT_DIR, "study2_success_cochrans_q.csv"), index=False)
    mcnemar_pooled.to_csv(os.path.join(OUT_DIR, "study2_success_mcnemar_pooled.csv"), index=False)
    mcnemar_per_model.to_csv(os.path.join(OUT_DIR, "study2_success_mcnemar_per_model.csv"), index=False)
    print(f"\nSaved CSVs -> {OUT_DIR}/")
