"""
Haiku-trial variant of harness/run_trial.py. Identical to the main-study
harness except that the system message is wrapped with an Anthropic
`cache_control: ephemeral` breakpoint for Claude models (see
_system_message / _is_claude_model below); this is gated to Claude only
and is a no-op for the four main-study models. Used solely for the
Study 4 Claude Haiku exploration (transformers/27 and the placebo run).
...
def _is_claude_model(model):
    """Anthropic prompt caching needs an explicit cache_control breakpoint
    on the message content -- unlike some other providers, it's never
    automatic. Gated to Claude models only so this can never change
    behavior for the four models already in the main study tree, even if
    litellm's handling of an unrecognized cache_control field differs by
    provider."""
    return "claude" in model.lower()


def _system_message(system_prompt, model):
    """Plain string content for every model except Claude, where the
    content is wrapped as a block list with an ephemeral cache_control
    breakpoint -- this is what actually activates prompt caching (merely
    resending identical text does not); see _cached_tokens() above, which
    only reads the resulting cache-hit stats, and never itself requested
    caching for any model."""
...
def main():
    ...
    messages = [
        {"role": "system",  "content": _system_message(system_prompt, args.model)},
