"""
Study 0 Goal 3 -- pooled input-tokens boxplot by map condition (none / ast
...
tables/thesis_appendix_map_comparison_{friedman,wilcoxon}.tex -- unlike
F1, input tokens showed a strong, near-unanimous map-condition effect
(Friedman p<.001; 12/12 models higher under either map than none per the
by-model breakdown), which this figure should make visually obvious as
three clearly separated distributions, not the near-identical boxes seen
for F1.
...
compared against it and against each other -- same convention as
plot_study0_map_f1_boxplot.py.
...
Writes figures/study0_map_input_tokens_boxplot.png.

Usage:
    python3 figures/scripts/plot_study0_map_input_tokens_boxplot.py
"""
import os
...
OUT_PNG = os.path.join(_ROOT, "figures", "study0_map_input_tokens_boxplot.png")
...
def main():
    ...
    data = [df.loc[df["map_type"] == m, "input_tokens"].dropna().values for m in MAP_ORDER]
    ...
    ax.set_ylabel("Input tokens per trial", fontsize=12)
