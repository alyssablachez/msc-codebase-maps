"""
Study 0 turn-count boxplot -- turns per trial per model, grouped into the 6
small/large pairs the study was designed around. Structurally identical to
plot_study0_cost_boxplot.py / plot_study0_time_boxplot.py (same pair
grouping, same small/large tint convention) so the figures read as a set;
only the metric and pair order differ.
...
Writes figures/study0_turns_boxplot.png.

Usage:
    python3 figures/scripts/plot_study0_turns_boxplot.py
"""
import os
...
OUT_PNG = os.path.join(_ROOT, "figures", "study0_turns_boxplot.png")

# Pair order: ranked by the LARGE model's median turns, ascending (mean as
# tiebreak -- Nemotron-3/Qwen3-VL tie on median). Recomputed from
# study_0/results_all.csv -- not a fixed convention, re-derive if the
# underlying turn-count data changes. Independent of the cost/time
# boxplots' PAIR_ORDER, which rank by different metrics.
PAIR_ORDER = ["Nemotron-3", "Qwen3-VL", "DeepSeek-V4", "Ministral", "gpt-oss", "GLM-4.7"]
PAIR_COLOR = {
    "Qwen3-VL":    "#2a78d6",
    "gpt-oss":     "#eb6834",
    "Ministral":   "#1baf7a",
...
def main():
    ...
    for pair, size in MODEL_ORDER:
        ...
        data.append(sub["turns"].dropna().values)
