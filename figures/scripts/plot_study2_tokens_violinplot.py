"""
Study 2 (on-demand/voluntary map tools vs. baseline) -- pooled
input-tokens violin plot by map condition, same convention as
figures/scripts/plot_study1_tokens_violinplot.py (values in millions, to
avoid matplotlib's scientific-notation axis offset), extended to
Study 2's 5 conditions (adds `all_tools`, slot-4 yellow #eda100).
`baseline` is Study 1's shared cross-study control.

Reads data/compiled_results_combined.pkl directly.
Writes figures/study2_tokens_violinplot.png.

Usage:
    python3 figures/scripts/plot_study2_tokens_violinplot.py
"""
import os
...
OUT_PNG = os.path.join(_ROOT, "figures", "study2_tokens_violinplot.png")
