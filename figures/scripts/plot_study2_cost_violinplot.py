"""
Study 2 (on-demand/voluntary map tools vs. baseline) -- pooled cost
violin plot by map condition, same convention as
figures/scripts/plot_study1_cost_violinplot.py, extended to Study 2's 5
...
Writes figures/study2_cost_violinplot.png.

Usage:
    python3 figures/scripts/plot_study2_cost_violinplot.py
"""
import os
...
OUT_PNG = os.path.join(_ROOT, "figures", "study2_cost_violinplot.png")
...
def main():
    ...
    data = [df.loc[df["map_condition"] == m, "total_cost"].dropna().values for m in MAP_ORDER]
