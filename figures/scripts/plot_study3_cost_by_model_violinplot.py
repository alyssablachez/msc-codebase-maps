"""
Study 3 (on-demand/required map tools vs. baseline) -- cost violin plot
broken out by BOTH model (x-axis clusters) and map condition (one violin
per condition within each cluster), same layout as
figures/scripts/plot_study2_cost_by_model_violinplot.py. Colour encodes map
condition (same fixed categorical hues as every other Study 3 chart);
model is the x-axis grouping dimension.
...
Writes figures/study3_cost_by_model_violinplot.png.

Usage:
    python3 figures/scripts/plot_study3_cost_by_model_violinplot.py
"""
import os
...
OUT_PNG = os.path.join(_ROOT, "figures", "study3_cost_by_model_violinplot.png")
