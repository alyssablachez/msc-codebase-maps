"""
Study 2 (on-demand/voluntary map tools vs. baseline) -- cost violin plot
broken out by BOTH model (x-axis clusters) and map condition (one violin
per condition within each cluster), same layout as
figures/scripts/plot_study2_tokens_by_model_violinplot.py. Colour encodes
map condition (same fixed categorical hues as every other Study 2
chart); model is the x-axis grouping dimension.
...
Writes figures/study2_cost_by_model_violinplot.png.

Usage:
    python3 figures/scripts/plot_study2_cost_by_model_violinplot.py
"""
import os
...
OUT_PNG = os.path.join(_ROOT, "figures", "study2_cost_by_model_violinplot.png")
...
def main():
    ...
    for group_pos, model in enumerate(MODEL_ORDER):
        for offset, cond in zip(OFFSETS, MAP_ORDER):
            vals = df.loc[(df["model"] == model) & (df["map_condition"] == cond), "total_cost"].dropna().values
            if len(vals) == 0:
                continue
