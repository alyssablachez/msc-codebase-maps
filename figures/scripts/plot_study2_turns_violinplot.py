"""
Study 2 (on-demand/voluntary map tools vs. baseline) -- pooled turns-used
violin plot by map condition, same convention as
figures/scripts/plot_study1_turns_violinplot.py, extended to Study 2's 5
conditions (adds `all_tools`, slot-4 yellow #eda100). `baseline` is
Study 1's shared cross-study control.

Reads data/compiled_results_combined.pkl directly.
Writes figures/study2_turns_violinplot.png.

Usage:
    python3 figures/scripts/plot_study2_turns_violinplot.py
"""
import os
...
OUT_PNG = os.path.join(_ROOT, "figures", "study2_turns_violinplot.png")
...
def main():
    ...
    data = [df.loc[df["map_condition"] == m, "turns_used"].dropna().values for m in MAP_ORDER]
