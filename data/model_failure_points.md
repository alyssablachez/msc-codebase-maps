...
- Five issues analyzed so far (`pandas/35`, `fastapi/17`, `thefuck/20`,
  `keras/12`, `yt-dlp/41`); intentionally kept broad and issue-specific
  rather than prematurely generalized — revisit once a handful more issues
  are logged here to see which patterns recur. The co-change-retrieved-and-
  ignored pattern (#6, #12) has now recurred across two separate issues and
  is the strongest repeat candidate for calling a real, general finding
  rather than an issue-specific quirk. Note `yt-dlp/41` has no genuine bug/
  fix behind it (confirmed via the underlying MULocBench annotation itself,
  which flags it as not really countable) — its entries (#14) are about
  evaluation/methodology, not model reasoning failures, and should be read
  differently from the other four issues' entries.
